...
No framework, no additional downloads, just pure "how do I call this?" reference
material from `main()` to animation and some basic raytracing effects.
...
Check if it works and runs before you proceed.
...
This simple example writes it directly into the output buffer, but it could be
...
Transforming this to world space is also relatively simple: the transformation
matrix is available "for free" through intrinsics.
...
To determine if this pixel should be in shadow, trace a ray toward the light
source.
...
Simply reduce brightness if the shadow ray hits anything.
...
Also create a fence that will be used for GPU-CPU synchronization, completing
this function.
...
Start by asking for a DXGI factory.
Microsoft would be sad if I used old, superseded DXGI 1.0, so let's use 1.3.
...
Ask for a double-buffered swap chain for the window we made in `main`.
...
The factory did its job and is no longer needed.
...
surface.
This is not used anywhere in the main article, but can be useful in the appendix.
...
This helper makes BLAS creation trivial.
...
This one is optimized for neither fast builds nor fast trace, but it's updatable.
...
Start with a helper that converts from a 4x4 row-major XMMATRIX to the
...
cube's shadow.
Tilt it back and forth sideways over time.
...
Build a root signature for our PSO later.
...
`ID3D12PipelineState`, because why not?
...
We'll provide PERFORM_UPDATE as a build flag and source acceleration structure.
...
* Use CallShader() and callables for ClosestHit.
