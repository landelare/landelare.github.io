...
Epic has a very restrictive stance on `auto`. While there are traps around it,
there are also situations where you can reduce code noise without making it
harder to understand. It's important to distinguish between "decorated" and
regular `auto`, see below.

There are virtually no drawbacks to using it when the type is obvious and it's
...
You can also consider using it when the type is not immediately visible but it's
still obvious what the variable will be:
...
using most of the time, especially in UE game code.
"Raw" `auto` and `auto&&` have more hidden traps and footguns:
...
`std::enable_shared_from_this`, if you're hitting asserts consider replacing it.
...
supposedly not prevented by it mostly don't exist. Overusing it adds noise and
takes attention away from the real, important `const`s.

Of course this doesn't apply to pointers or references to `const` things, where
...
### ...
`...` varargs are legacy, crash-prone, not type-safe, and all around horrible.
...
Also, don't forget that [Hot Reload Sucks](http://www.hotreloadsucks.com/)!
